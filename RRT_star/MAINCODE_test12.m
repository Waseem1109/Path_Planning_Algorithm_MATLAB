...
while (data(11) ~= 0)
    ...
    if (data(8) == 1234)
        ...
        distance = data(7); thetaUltra = data(10); distance1 = data(14); distance2 = data(15);
        [distance, distance1, distance2, ob1, ob2, ob3] = obstacle_filter(distance, distance1, distance2, theta_C, thetaUltra, x_C, y_C, Obstacles, dthresh,obstacle_thickness);
        [map, Obstacles] = Ultrasonic_map1(map, distance, distance1, distance2, threshhold, Obstacles, ob1, ob2, ob3,obstacle_thickness);
    else
        data(11) = 1;
    end
    if ((toc - prevtime) > 1)
    ...
    while ((ErrorX > EL_thresh))
        ...
        while ((ErrorX > ES_thresh))
            ...
            if (data(8) == 1234)
                ...
                if(ErrP<10)
                    ddthresh=5;dd1thresh=5;dd2thresh=5;
                end
                if ((distance < ddthresh && distance > 5) && data(11) == 0 && data(12) == 0 && RP == 0)  || (distance1 < dd1thresh && distance1 > 5) ||(distance2 < dd2thresh && distance2 > 5)
                    show(map)
                    PLOTBOT(x_C, y_C, r, theta_C, p, thetaUltra);
                    plot(path(:, 1), path(:, 2), 'Color', 'm', 'LineWidth', 2)
                    data(11) = 1;
%                     if(distance < ddthresh)
%                         while (data(11) ~= 0)
%                             flush(s);
%                             write(s, [point theta_N M RP LP], 'single');
%                             data = read(s, 15, 'single');
